...
void Ship::mvdown()
{
    ...
    if(yLoc > yMax-4) // bc bottom border is ymax-1, ymax-2 is right above bottom border
        yLoc = yMax-4;
}
    ...
    switch(choice)
    {
        ...
        case 'k':
            ...
            if(next_y != yMax)
            {
                ...
                mvwprintw(curwin, yLoc-2, xLoc, "$");  //added mvwprintw and curwin
                //mvwprintw(curwin, yLoc, xLoc, " "); // added
                wrefresh(curwin);
            
                usleep(DELAY);
                
                next_y = y - direction_y;
                y-= direction_y; // y+ y- bc bullet is moving up the y axis towards 0
